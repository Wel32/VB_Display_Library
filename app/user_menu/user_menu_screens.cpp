...
void* Screen3::Select()
{
	...
	uint8_t text_set_pos = 0;

	for (uint16_t j = 1; j < 2000; j++)
	{
		if (!(j % 200))
		{
			text_color_i = modulo_addition(text_color_i, +1, sizeof(text_color)/sizeof(uint32_t));

			int16_t tx = ScreenWidth / 2, ty = text_start_y;
			if (++text_set_pos == 1) tx +=40;
			else if (text_set_pos == 2) ty +=40;
			else if (text_set_pos == 3) tx -=40;
