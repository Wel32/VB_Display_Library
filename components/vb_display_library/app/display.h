...
public:
    ...
    void common_draw(rect mask, uint32_t end_layer);
    ...
    void clear_data();
    void set_or_update_obj(std::unique_ptr<draw_obj> obj, uint32_t* layer_num_store, bool update_on_the_screen = 1, bool update_obj_if_exist = 1);
    void update_obj_on_screen(std::unique_ptr<draw_obj> new_obj, uint32_t img_layer);

    void pin_layer_buffer_to_obj(uint32_t obj_layer, uint32_t* layer_num_store);
    void change_obj_layer(uint32_t obj_layer, uint32_t new_layer);
    
    void draw_new_obj(uint32_t img_layer);
    ...
    void screen_buf_insert_obj(std::unique_ptr<draw_obj> obj, uint32_t* layer_num_store, uint32_t layer = (uint32_t)(-1));
