    ...
    VBDL_tImage((const tImage*)NULL, 0, 0, 0, 0, 0)
{
    ...
    png_buf = upng_new_from_bytes((const unsigned char*)outfile.addr, outfile.size);
    upng_t* upng = (upng_t*)png_buf;
	...
	if (upng != NULL)
	{
        ...
        if (upng_get_error(upng) == UPNG_EOK)
        {
		    ...
		    tImage buf_image = {(const uint8_t*)upng_get_buffer(upng), width, height, depth};
...
VBDL_uPNG::~VBDL_uPNG()
{
    if (png_buf != NULL) upng_free((upng_t*)png_buf);
}
